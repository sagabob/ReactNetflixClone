...
import {
  ...
  NavbarDropDownProps,
  NavbarGroupProps,
  NavbarLeftMenuAreaProps,
  NavbarLeftMenuItemProps,
  NavbarLeftMenuProps,
  NavbarLogoProps,
  NavbarPictureProps,
  NavbarProfileProps,
  NavbarRightMenuAreaProps,
  NavbarRightMenuButtonProps,
  NavbarSearchProps,
  NavbarTextLinkProps,
} from "./types";
...
Navbar.LeftMenu = ({ children, ...restProps }: NavbarLeftMenuProps) => {
  return <LeftMenu {...restProps}>{children}</LeftMenu>;
};

Navbar.LeftMenuItem = ({ children, ...restProps }: NavbarLeftMenuItemProps) => {
  return <LeftMenuItem {...restProps}>{children}</LeftMenuItem>;
};

Navbar.ButtonLink = ({ children, to, ...restProps }: NavbarButtonLinkProps) => {
  return (
    <ButtonLink to={to} {...restProps} style={({ isActive }) => (isActive ? { fontWeight: 700 } : undefined)}>
      {children}
    </ButtonLink>
  );
};
...
Navbar.RightMenuButton = ({ children, ...restProps }: NavbarRightMenuButtonProps) => {
  return <RightMenuButton {...restProps}>{children}</RightMenuButton>;
};
